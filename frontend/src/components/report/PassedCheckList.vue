<template><div class="checks"><div v-for="item in items" :key="item.check_id"><span>✓</span><p><strong>{{ item.title }}</strong><small>{{ item.description }}</small></p></div></div></template>
